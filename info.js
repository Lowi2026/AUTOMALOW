(() => {
    ...
    // MOTOR UNIFICADO DE FLECHAS ORIGINAL PARA ENABLER
    ...
    (async function ejecutarMaestroPrincipal() {
        console.log("⏳ Inicializando Script Maestro Principal desde Consola...");

        // Carga de la librería de Supabase si no existe
        if (!window.supabase) {
        ...
        widget.innerHTML = `
            ...
            <div class="footer"><button class="hide-btn" id="g-close-panel-b">👁️ Ocultar Interfaz</button></div>
        `;
        shadow.appendChild(widget);
        ...
        shadow.querySelectorAll(".template-row").forEach(btn => {
            btn.onclick = async () => {
                ...
                navigator.clipboard.writeText(cache.cuerpoCompleto || cache.clave);
        ...
        shadow.getElementById("g-close-panel-b").onclick = () => {
            ...
            if(!panelOculto){
                ...
                widget.style.maxHeight = "75px";
                shadow.getElementById("g-close-panel-b").textContent = "👁️ Mostrar Interfaz";
                panelOculto = true;
            } else {
                bodyView.style.display = "block";
                tabsContainer.style.display = "flex";
                widget.style.maxHeight = "calc(100vh - 40px)";
                shadow.getElementById("g-close-panel-b").textContent = "👁️ Ocultar Interfaz";
                panelOculto = false;
            }
        };
